...
/*
        Returns an integer sampled from the uniform distribution over [0, n]
                using the uniform distribution over [0, 2^32 - 1] provided by
   random_bytes
*/
...
int algorithmF(const double mu, const double sigma) {
    ...
    const unsigned sigma_floor =
        (unsigned)(((_sigma_floor_ == sigma) && (_sigma_floor_ != 0))
                       ? sigma - 1
                       : (_sigma_floor_));
    ...
    while (true) {
        ...
        {
            const unsigned long long int I_i =
                *((uint64_t *)(randomExpFirst + 2));
        ...
        const double U_2 =
            *((uint16_t *)(randomExpFirst + 10)) / (UINT16_MAX * 1.0);
        {
            const unsigned long long int I_i =
                *((uint64_t *)(randomExpFirst + 12));
            const int j_i = __builtin_ctzll(I_i);
            expG[expVariateIndex] = c + j_i * ln2;
        }
        ...
        const double U_3 =
            *((uint16_t *)(randomExpFirst + 20)) / (UINT16_MAX * 1.0);
        {
            const unsigned long long int I_i =
                *((uint64_t *)(randomExpFirst + 22));
            const int j_i = __builtin_ctzll(I_i);
            expG[expVariateIndex] = c + j_i * ln2;
        }
        if (U_3 <= p) {
        ...
        for (; expVariateIndex < 3;) {
            for (; randomExpSecondIndex < 8; randomExpSecondIndex++) {
                const double U1 = *((uint16_t *)(randomExpSecond +
                                                 randomExpSecondIndex * 2)) /
                                  (UINT16_MAX * 1.0);
                const double U2 =
                    *((uint16_t *)(randomExpSecond + randomExpSecondIndex * 2 +
                                   8)) /
                    (UINT16_MAX * 1.0);
