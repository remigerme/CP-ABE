...
#define PARAM_M \
    (PARAM_D *  \
     (PARAM_K + \
      2))  // parameter m ( = d(k+2) for the computational instantiation)
// #define PARAM_ALPHA (10.0 * PARAM_SIGMA) // Gaussian parameter (sampling
// perturbations)
...
#define PARAM_N 256 // degree of polynomials
#define PARAM_R 8 // number of irreducible factors of x^n + 1 in F_q[x]
#define SMALL_DEGREE (PARAM_N / PARAM_R) // degree of irreducible factors of x^n
+ 1 in F_q[x]
*/
...
// #define DOUBLE_ZERO (((double_scalar) PARAM_Q) << (8*sizeof(double_scalar) -
// PARAM_K - 1)) // a double_scalar that is 0 mod q, such that there will be no
// underflow nor overflow during computations
#define DOUBLE_ZERO             \
    (((double_scalar)PARAM_Q) * \
     PARAM_Q)  // a double_scalar that is 0 mod q, such that there will be no
               // underflow nor overflow during computations
#define SIGNED_DOUBLE_ZERO \
    (((double_scalar)PARAM_Q) << (8 * sizeof(double_scalar) - PARAM_K - 2))
...
/*
        ...
        #error "(n/r)*q^2 >= 2^63, there might be overflow in polynomial
multiplication" #endif
*/
...
// Matrix of signed polynomials, used when sampling since Gaussian values are
// signed
...
#define triangular_poly_matrix_element(M, i, j) \
    (&M[(PARAM_N) * ((i) * ((i) + 1) / 2 + (j))])

extern cplx cplx_roots_of_unity[2 * PARAM_N - 1];

// #define cplx_root_of_unity_of_order(k, i) (cplx_roots_of_unity[(2 * (PARAM_N)
// / (k) * (i))])

#define cplx_cyclotomic_factorisation_tree(i, j) \
    (cplx_roots_of_unity[(1 << (i)) + (j)-1])
